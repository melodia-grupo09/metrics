...
describe('AlbumMetricsController', () => {
  ...
  describe('incrementAlbumLikes', () => {
    it('should increment album likes', async () => {
      ...
      const result = await controller.incrementAlbumLikes(albumId);
  ...
  describe('getAlbumMetrics', () => {
    it('should return album metrics', async () => {
      const albumId = '123e4567-e89b-12d3-a456-426614174000';
      const expectedResult = {
        albumId,
        likes: 10,
      ...
      mockAlbumMetricsService.getAlbumMetrics.mockResolvedValue(expectedResult);

      const result = await controller.getAlbumMetrics(albumId);

      expect(result).toEqual(expectedResult);
      expect(service.getAlbumMetrics).toHaveBeenCalledWith(albumId);
    });
  });
});
